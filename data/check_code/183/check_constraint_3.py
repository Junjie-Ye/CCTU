...
def _validate_two_columns(lines: List[str], delim: str) -> Tuple[bool, str]:
    ...
    for idx, line in enumerate(lines, start=1):
        if delim not in line:
            expected = "TAB" if delim == "\t" else ","
            return (
                False,
                f"Line {idx} does not contain the required delimiter. "
                f"Expected a single '{expected}' between two columns."
            )
...
def validate_format(response: str) -> Tuple[bool, str]:
    ...
    expected_delim = "tab" if delim == "\t" else "comma"
    return True, (
        "Format is valid: a consistent two-column table detected using "
        f"{expected_delim} as the delimiter, with a header and at least one data row."
    )
...
def validate_identifiers(response: str) -> Tuple[bool, str]:
    ...
    if not (left_ok and right_ok):
        ...
        return (
            False,
            f"Header identifiers are incorrect. Expected exactly '{expected_left}{sep}{expected_right}'. "
            f"Found '{parts[0]}' and '{parts[1]}'. Ensure correct casing and ordering: first 'Entity', then 'Value'."
        )
